...
$candidates = @(
    @{ exe = 'ticklog_custom_drop.exe'; name = 'ticklog-custom-drop'; out = 'windows_ticklog_custom_drop_r14.json'; extra = @() },
    @{ exe = 'ticklog_rtrb_drop.exe';   name = 'ticklog-rtrb-drop';   out = 'windows_ticklog_rtrb_drop_r14.json';   extra = @('--chunk-size','64') },
    @{ exe = 'ticklog_quill.exe';       name = 'ticklog-quill';       out = 'windows_ticklog_quill_r14.json';       extra = @() },
    @{ exe = 'ticklog_nanolog.exe';     name = 'ticklog-nanolog';     out = 'windows_ticklog_nanolog_r14.json';     extra = @() }
)
...
if (Test-Path -LiteralPath $cppQuill) {
    $json = Join-Path $outDir 'windows_quill_cpp_r14.json'
    $log  = Join-Path $outDir 'windows_quill_cpp_r14.log'
    Remove-Item -LiteralPath $json,$log,"$log.err" -Force -ErrorAction SilentlyContinue
    ...
    Write-Output '  -> windows_quill_cpp_r14.json'
}
